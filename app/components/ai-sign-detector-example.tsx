...
export default function AISignDetectorExample() {
    ...
    try {
      ...
      if (detection && detection.landmarks.length === 21) {
        ...
        if (useWasm && mlRecognizer) {
          ...
          console.log(`🚀 WASM 실행: ${executionTime.toFixed(3)}ms - "${recognition.gesture}" (${(recognition.confidence * 100).toFixed(1)}%)`);
        } else {
          // JavaScript 모드 (복잡한 픽셀 단위 연산)
          const startTime = performance.now();
          recognition = recognizeWithJavaScript(detection.landmarks);
          const endTime = performance.now();
          executionTime = endTime - startTime;
          
          console.log(`🔥 JS 복잡 연산: ${executionTime.toFixed(3)}ms - "${recognition.gesture}" (${(recognition.confidence * 100).toFixed(1)}%)`);
          
          // 매 10프레임마다 상세 성능 분석 출력
          if (performanceStats.jsCount % 10 === 0 && performanceStats.jsCount > 0) {
            console.log(`💡 JS 연산 복잡도: 32x32 픽셀 그리드, 가우시안 블러, Sobel 엣지 검출, 벡터 각도 계산`);
          }
        }
  ...
  const recognizeWithJavaScript = (landmarks: HandLandmark[]): RecognitionResult => {
    ...
    // 손바닥 중심점 계산 (웨이트 기반)
    const palmCenter = landmarks.slice(0, 5).reduce((acc, point, i) => {
    ...
    const gridSize = 32;
    const handGrid = new Array(gridSize).fill(0).map(() => new Array(gridSize).fill(0));
    ...
    // 가우시안 블러 적용 (픽셀 단위 연산)
    const blurredGrid = new Array(gridSize).fill(0).map(() => new Array(gridSize).fill(0));
    ...
    const features = {
      ...
      avgFingerAngle: fingerAngles.reduce((a, b) => a + b, 0) / fingerAngles.length,
      palmArea: palmArea,
    ...
    Object.entries(weights).forEach(([gesture, w], id) => {
      ...
      if (score > bestScore && confidence > 0.4) { // 최소 신뢰도 40%
        bestScore = score;
        bestGesture = gesture;
          ...
          {isRecording && (
            <div className={styles.currentStatus}>
              ...
              <div className={styles.statusText}>
                {useWasm ? "🚀 WASM" : "🔥 JS 복잡연산"} 모드로 {currentResult
                ...
                {(performanceStats.wasmCount > 0 || performanceStats.jsCount > 0) && (
                  <div className={styles.performanceStats}>
                    ...
                    {performanceStats.wasmCount > 0 && (
                      <div>
                        🚀 WASM 평균: {(performanceStats.wasmTime / performanceStats.wasmCount).toFixed(2)}ms
                        ({performanceStats.wasmCount}회)
                      </div>
                    )}
                    {performanceStats.jsCount > 0 && (
                      <div>
                        🔥 JS 복잡연산 평균: {(performanceStats.jsTime / performanceStats.jsCount).toFixed(2)}ms
                        ({performanceStats.jsCount}회)
                      </div>
                    )}
                    {performanceStats.wasmCount > 0 && performanceStats.jsCount > 0 && (
                      <div>
                        ⚡ WASM이 {((performanceStats.jsTime / performanceStats.jsCount) / (performanceStats.wasmTime / performanceStats.wasmCount)).toFixed(1)}x 빠름
                      </div>
                    )}
                  </div>
                )}
