...
```
                [Camera Input]
                      │
                      ▼
                [MediaPipe Hands] ──► 21 Landmarks (JavaScript)
                      │
                      ▼
                [WASM Interface] ──► Heap Memory Copy (JS → C++)
                      │
                      ▼
┌───────────────────────────────────────────────┐
│           WASM Core (C++ Engine)              │
│                                               │
│            [Data Preprocessing]               │
│      (Normalization & Relative Coords)        │
│                    │                          │
│         ┌──────────┴──────────┐               │
│         ▼                     ▼               │
│   ┌─────────────┐       ┌───────────┐         │
│   │   Track A   │       │  Track B  │         │
│   │ Rule-based  │       │ MLP Model │         │
│   └─────┬───────┘       └─────┬─────┘         │
│         │                     │               │
│    (Geometric)           (Inference)          │
│         │                     │               │
└─────────┼─────────────────────┼───────────────┘
          ▼                     ▼
    [Decision Maker (Priority & Confidence)]
                   │
                   ▼
       [Final Gesture ID & Name]
```
...
|  작업  |   JavaScript   |    C++ (WASM)    |     성능 향상     |
| :------: | :----------------: | :----------------: | :----------------: |
| 단일 제스처 인식  |      <1ms      |       <3ms      |       3배        |
| MLP 추론  |      <1ms      |       <4ms       | 4배 |
| 배치 처리 (10 프레임)  |      <5ms      |       <50ms      | 10배 |
| 대용량 행렬 연산  |      <21ms     |       <400ms     | 19배 |
